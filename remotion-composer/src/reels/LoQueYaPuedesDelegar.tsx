...
/**
 * LoQueYaPuedesDelegar - reel 2/4 (didactico) de la serie
 * reels-abogados-serie. Runtime HIBRIDO (los 3 ejes se presentan en el
 * proposal, ver decision_log d-001): el bloque 'checklist_delegable' (5
 * items EXACTOS de las secciones 2-6 de 'Tareas que ya se pueden delegar
 * a la IA' del post, excluyendo intake que ya se dramatiza aparte en
 * intake_beat) se autora en HYPERFRAMES (HTML/CSS puro), NO en Remotion
 * - mismo patron GO'd en LoQueCambiaEnTuClinica.tsx de la serie clinicas.
 * Se renderiza aparte en el CT 128 (hf lint . && hf render, comando
 * canonico de la skill aetherlogik-hyperframes) y se compone aqui como UN
 * clip MP4 (tareasDelegablesSrc).
 *
 ...
 * checklist_delegable 480f (20.0s) [clip HyperFrames] + caveat 120f (5.0s)
 * + brand_close 192f (8.0s) = 1104f = 46.000s exacto (tail_padding_seconds:
 * 0 en props).
 *
 * Ver LoQueYaPuedesDelegar.art-direction.md, .decision_log.json,
 * .scene_plan.json y .gates.json (auto-score pre-render).
 */
export interface LoQueYaPuedesDelegarProps {
...
export const LO_QUE_YA_PUEDES_DELEGAR_TOTAL_FRAMES = S1 + S2 + S3 + S4 + S5; // 1104
...
const INTAKE_TAGS: QualifyTag[] = [
  { label: "Info basica", value: "Tipo de caso, urgencia, ubicacion" },
  { label: "Calificacion", value: "Entra en tu area de practica?" },
  { label: "Agenda", value: "Consulta inicial directo en tu calendario" },
  { label: "Documentos", value: "Formularios y checklist antes de la reunion" },
];

export const LoQueYaPuedesDelegar: React.FC<LoQueYaPuedesDelegarProps> = ({
  tareasDelegablesSrc,
  musicSrc,
  logoSrc,
  sfxWhooshSrc,
  ...
  return (
    <AbsoluteFill style={{ background: BRAND.bg }}>
      ...
      <Sequence from={START1} durationInFrames={S1} name="cold_open">
        <KineticHeadline
          lines={["Estas tareas", "ya se pueden delegar", "a la IA."]}
          accentWord="delegar"
          fontSize={52}
          position="center"
        />
      ...
      <Sequence from={START2} durationInFrames={S2} name="intake_beat">
        ...
        <HeadlineOverlay text="Intake y calificacion de clientes" position="bottom" scrim fontSize={28} />
      </Sequence>

      {/* Scene 3 - checklist_delegable. Clip HyperFrames, mismo patron que */}
      {/* LoQueCambiaEnTuClinica.tsx de la serie anterior. */}
      ...
      {/* Scene 4 - caveat. Fiel al post: la IA no reemplaza la consulta. */}
      <Sequence from={START4} durationInFrames={S4} name="caveat">
        <KineticHeadline
          lines={["Esto no reemplaza", "la consulta personal.", "La hace mas productiva."]}
          accentWord="productiva"
          fontSize={44}
          position="center"
        />
      </Sequence>
      ...
      <Sequence from={START5} durationInFrames={S5} name="brand_close">
        <FilmGrade vignette={0.35} grainOpacity={0.1}>
          <BrandClose
            ...
            url="aetherlogik.com/blog . lo que la IA NO puede hacer, en el proximo video ->"
            logoSrc={logoSrc}
